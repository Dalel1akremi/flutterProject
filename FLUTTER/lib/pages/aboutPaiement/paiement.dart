  ...
  Future<void> createCommande() async {
    try {
      ...
      var response = await http.post(
        Uri.parse(
            'http://192.168.2.61:3000/createCommande?id_user=$userId&id_rest=$idRest'),
        body: jsonEncode({
          'id_items': idItems,
        }),
...
Future<void> makePaymentWithCreditCard() async {
  if (selectedCreditCard != null) {
    try {
      ...
      var response = await http.post(
        Uri.parse(
            'http://192.168.2.61:3000/recupererCarteParId?email=${authProvider.email}&cardId=$cardId'),
    ...
    if (userEmail != null) {
      try {
        var response = await http.get(Uri.parse(
            'http://192.168.2.61:3000/recupererCartesUtilisateur?email=$userEmail'));
        if (response.statusCode == 200) {
