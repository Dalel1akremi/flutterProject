...
const Produits = () => {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      ...
      try {
        ...
        setIsLoading(true);
        const response = await axios.get(`http://192.168.1.6:3000/getItemsByRestaurantId?id_rest=${id_rest}`);
  ...
  const handleArchivedToggle = async (itemId: string, currentValue: boolean) => {
    try {
      ...
      await axios.put(`http://192.168.1.6:3000/ArchiverItem/${itemId}`, { isArchived: !currentValue });
    } catch (error) {
  ...
  const getNomStepById = async (id_Step: number) => {
    try {
      ...
      const response = await axios.get(`http://192.168.1.6:3000/getNomStepById/${id_Step}`);
  ...
  const handleEditItem = async () => {
    try {
      ...
      setIsLoading(true);
      await axios.put(`http://192.168.1.6:3000/updateItem?itemId=${_id}`, {
        nom: editedItem?.nom,
        prix: editedPrix,
      ...
      const response = await axios.get(`http://192.168.1.6:3000/getItemById?itemId=${_id}`);
