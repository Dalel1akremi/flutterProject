...
export default function AddItem() {
  const [formData, setFormData] = useState<FormData>({
    nom: '',
    type: '',
    prix: 0, // Définir une valeur par défaut
    description: '',
    quantite: 0, // Définir une valeur par défaut
    max_quantite: 0, // Définir une valeur par défaut
    is_Menu: false,
    is_Redirect: false,
    id_cat: '',
    id_Steps: '',
                  ...
                  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
                    ...
                    try {
                      ...
                      for (const key in formData) {
                        ...
                        if (value !== null) {
                          if (key === 'image') {
                            // Si la clé est 'image', ajoutez l'image en tant que fichier à envoyer
                            formDataToSend.append(key, value as File);
                          } else {
                            // Sinon, ajoutez la valeur convertie en chaîne à envoyer
                      ...
                      // Si la réponse est réussie (200 OK), affichez le message de succès de l'API dans une alerte verte
                      alert(response.data.message);
                      
                      // Réinitialisation du formulaire après le succès
                      setFormData({
                        nom: '',
                        type: '',
                        prix: 0,
                      ...
                      // Si une erreur se produit, affichez le message d'erreur de l'API dans une alerte rouge
