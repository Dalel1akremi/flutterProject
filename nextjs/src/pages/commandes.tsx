...
const CommandesPage = () => {
  ...
  const handleStateChange = async (id: string, newState: string) => {
    try {
      ...
      const response = await axios.put(`http://192.168.2.65:3000/commandes?commandeId=${id}&newState=${newState}`);
  ...
  const sendNotification = async (idRest: string | null) => {
    try {
      ...
      const response = await axios.post(`http://192.168.2.65:3000/sendNotification?id_rest=${idRest}`);
