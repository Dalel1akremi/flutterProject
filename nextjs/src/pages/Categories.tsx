...
const CategoriesPage = () => {
  ...
  const getCategories = async (id_rest: string) => {
    try {
      const response = await axios.get(`http://192.168.2.61:3000/getCategoriesAd?id_rest=${id_rest}`);
  ...
  const handleArchivedToggle = async (_id: string, isArchived: boolean) => {
    try {
      ...
      const response = await axios.put(`http://192.168.2.61:3000/ArchivedCategorie/${_id}`, { isArchived: newIsArchived });
  ...
  const handleUpdateCategory = async (id_cat: string) => {
    try {
      await axios.put(`http://192.168.2.61:3000/updateCategory/${id_cat}`, { nom_cat: newCategoryName });
